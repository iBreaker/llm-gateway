...
export class SupabaseAdapter implements DatabaseAdapter {
  ...
  async raw<T>(sql: string, params?: any[]): Promise<T[]> {
    ...
    try {
      ...
      if (isDDL) {
        // 尝试使用 exec_sql RPC 函数执行 DDL
        try {
          const { error } = await this.client.rpc('exec_sql', { sql })
          if (error) {
            console.warn(`⚠️ DDL 执行失败，可能需要手动执行: ${sql.substring(0, 100)}...`, error)
            return [] as T[]
          }
          console.log(`✅ DDL 执行成功: ${sql.substring(0, 100)}...`)
          return [] as T[]
        } catch (rpcError) {
          console.warn(`⚠️ exec_sql RPC 不可用，DDL 语句需要手动执行: ${sql.substring(0, 100)}...`)
          return [] as T[]
        }
      }
      
      // 对于 SELECT 查询，尝试使用 RPC 执行
      if (sql.trim().toLowerCase().startsWith('select')) {
        try {
          ...
          console.warn('⚠️ exec_sql RPC 不可用，无法执行 SELECT 查询')
          return [] as T[]
        }
      }
      
      // 对于其他 DML 语句，也尝试使用 RPC
      try {
        const { error } = await this.client.rpc('exec_sql', { sql })
        if (error) throw error
        return [] as T[]
      } catch (rpcError) {
        console.warn('⚠️ exec_sql RPC 不可用，无法执行 SQL 语句')
        return [] as T[]
      }
    } catch (error) {
